...
/*-------------------------------------------------------------------------
  ...
  This file contains AES system equiped with masked clearing circuitry
  Takes stop_detect & delayed_edge inputs from Borrowed Time clock monitor &
  delayed edge generation circuitry in parent module

 -------------------------------------------------------------------------*/

//================================================ AES_Composite_enc
module AES_Composite_enc
  (Kin, Din, Dout, Krdy, Drdy, Kvld, Dvld, EN, BSY, CLK_IN, RSTn, stop_detect, delayed_edge);
   ...
   input		  stop_detect;	// BT
   input		  delayed_edge; // BT
   ...
   wire           rst;
	...
	wire [79:0]		rng_key, iv;
	
	reg [3:0]		cnter = 4'b0000;
	
	wire		    sys_clk;
	
	
	// BT: hard coded initialisers for RNG.
   //     In any real system deployment:
   //     - rng_key should come from a TRNG 
   //     - IV can be public, but should take a
   //       new value for each encryption session

	assign rng_key = 80'hAAAAAAAAAAAAAAAAAAAA;
	assign iv = 80'hCCCCCCCCCCCCCCCCCCCC;

   //------------------------------------------------
   assign rst = ~RSTn;
	
	always @(posedge sys_clk) begin    // BT for Trivium initialisation 
		if (~cnter[3])		cnter <= cnter + 4'b1;
	end
   ...
   KeyExpantion keyexpantion 
	...
	Trivium trivium // BT: source of randomness
		(.clk(sys_clk), .rst(cnter[2]), .key(rng_key), .iv(iv), .stream_out(rng_output));
 
 
// BT:  clk_mux as in paper (Figure 4)
...
clk_mux (
   ...
   .I0(CLK_IN),           // 1-bit input: Primary clock
   .I1(delayed_edge),           // 1-bit input: Secondary clock
   .IGNORE0(1'b1), // 1-bit input: Clock ignore input for I0
   .IGNORE1(1'b1), // 1-bit input: Clock ignore input for I1
   .S0(~stop_detect),           // 1-bit input: Clock select for I0
   .S1(stop_detect)            // 1-bit input: Clock select for I1
);
// End of BUFGCTRL_inst instantiation
   ...
   always @(posedge sys_clk or posedge rst) begin
      if (rst)                 dat <= 128'h0;
		else if (stop_detect)  dat <= rng_output; // BT: masked clear
      else if (EN) begin
         if (Drdy)             dat <= Din ^ key;
